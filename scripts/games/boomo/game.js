...
var nwrap = require('../../utility').nwrap;
...
Game.prototype.kickPlayer = function(user) {
    if(this.player.keys().includes(user.id)){
        this.delete(user.id)
    }
};
...
Game.prototype.playersOrdered = function() {
    ...
    for (var p of this.players.values()) {
        if ((p.user.connected || p.in_game) && order.indexOf(p) == -1) {
            order.push(p);
        }
    }
    return order;
...
Game.prototype.getUpdate = function(user, data) {
...
Game.prototype.process = function(user, data){
    var player = this.getPlayer(user)
    switch(data.type){
        case 'start':
            this.startGame()
        ...
        case 'playCard':
            ...
            this.playCard(player, data.card)
            return this.getUpdate(player)
    }
}
