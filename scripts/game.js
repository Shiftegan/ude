...
function Game() {
    ...
    this.keepAwake = function(player) {
    ...
    this.getPlayers = function(){
        ...
        for (var player of this.players){
            if(player[1].in_game){players.push(player[1])}
        }
        return players
    }
    ...
    // this.playerOrderd = function(index) {
    ...
    this.getCards = function(player, restrictions) {
        console.log("RECEIVED QUERY:", player.name, restrictions)
        restrictions = utility.merge({locations:["purchase", "hand"], maxCost: 999, type: false, invalid: [], playable: false}, restrictions)
        console.log("MADE QUERY:", player.name, restrictions)
        valid = []
    ...
    this.buyCard = function(player, card){
        ...
        if (player != this.active_player || this.phase != "buy") {
            console.log(player.name, 'tried to buy out of turn');
        } else {
            if (this.purchase[card.name] && card.cost <= player.current_gold && player.buys > 0) {
                ...
                console.warn(player.name, 'tried to buy a card they were not allowed to.');
            }
        }
    ...
    this.makeChoice = function(player, choice, type){
        ...
        if (player != this.active_player || this.phase != "choice") {
            console.log(player.name, 'tried to choose out of turn');
        } else {
            if (this.otherChoices.includes(choice) || this.cardChoices.includes(choice.name)) {
                ...
                console.warn(player.name, 'tried to make a choice they were not allowed to.');
            }
        }
    ...
    this.getPlayerVision = function(player) {
